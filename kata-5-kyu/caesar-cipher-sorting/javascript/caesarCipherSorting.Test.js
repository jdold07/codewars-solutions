...
const { caesarSort } = require("./caesarCipherSorting")
...
describe("Caesar Cipher Sorting", () => {
  ...
  const test = (input) => assert.deepEqual(sort(caesarSort(Array.from(input))), sort(refCaesarSort(input)))
  it("Fixed Tests", () => {
    test(["a", "b", "c"])
    test(["ab", "ac"])
    test(["z", "a", "az", "zy"])
  ...
  const list = (m) => (n) =>
    randomize(
      concat(
        Array.from({ length: m }, () => {
          const w = word(n)
          return Array.from({ length: rnd(13) + 1 }, (_, i) => caesar(i)(w))
        })
      )
    )

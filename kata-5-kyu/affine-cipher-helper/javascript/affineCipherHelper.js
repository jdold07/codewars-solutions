...
class AffineCipher {
  constructor(abc, a, b) {
    if (this.gcd(a, abc.length) !== 1) throw new Error(`${a} is not co-prime with ${abc.length}`)
    this.abc = abc
    this.a = a
    this.b = b
...
class AffineCipher2 {
  constructor(abc, a, b) {
    ...
    this.a = this.gcd(abc.length, a) == 1 ? a : this._aError() // non-negative integer co-prime with the alphabet length
    this.b = b // any non-negative integer
    this.m = this.abc.length // length of the alphabet used in the object instance
  }
  _aError() {
    throw new Error("a is not co-prime with m")
  }
  _x(letter) {
...
module.exports = { AffineCipher, AffineCipher2 }

...
const keyGen = (strKey) => strKey
  ...
  function trifidDecode(strKey, bp, data) {
    const key = keyGen(strKey)
    const chunk = data.match(RegExp(".{1," + bp + "}", "g")).map((v) => v
      .split("")
      .map((v) => key[v].join(""))
      .join(""))

...
describe("Sum of Pairs", function () {
  ...
  it("Excruciatingly Long List Tests", function () {
    ...
    assert.deepEqual(
      solution(l9, 13),
      [6, 7],
      "Five Million Numbers With Middle Pair: Should terminate with a valid pair output"
    )
    assert.deepEqual(
      solution(l9, 5),
      [8, -3],
      "Five Million Numbers With Pair At End: Should terminate with a valid pair output"
    )
    assert.deepEqual(
      solution(l9, 16),
      [13, 3],
      "Five Million Numbers With Pair At Start: Should terminate with a valid pair output"
    )
    assert.deepEqual(
      solution(l9, 31),
      undefined,
      "Five Million Numbers With No Match: Should return undefined in a decent time period"
    )
  })
})

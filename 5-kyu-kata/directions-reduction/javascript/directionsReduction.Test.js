...
describe("tests suite", function () {
	...
	it("random tests", function () {
		const rr = [["NORTH", "EAST"], ["EAST", "SOUTH"], ["SOUTH", "WEST"], ["WEST", "NORTH"], ["NORTH", "NORTH", "NORTH"]];

		shuffle(rr).forEach(function (res) {
			let aa = ["NORTH", "SOUTH"], bb = ["EAST", "WEST"];
			const rnd = function () { return ~~(Math.random() * 2) };
			const input = res.slice(0);

			for (let x = 0, z = 2 + rnd(); x < z; x++) {
				let a = rnd(), b = rnd();
				if (x % 2) {
					input.push(aa[a]); input.push(aa[(a + 1) % 2]);
					input.unshift(bb[b]); input.unshift(bb[(b + 1) % 2]);
				} else {
					input.push(bb[b]); input.push(bb[(b + 1) % 2]);
					input.unshift(aa[a]); input.unshift(aa[(a + 1) % 2]);
				}
			}
			doTest(input, res);
		});

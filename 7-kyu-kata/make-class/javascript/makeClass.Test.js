...
describe("Some random Tests", function() {
  for (let i = 0; i < 50; i++) {
    ...
    Test.assertEquals(JSON.stringify(o),JSON.stringify(p),
...
describe("Some more random Tests", function() {
  ...
  for (let i = 0; i < 20; i++) {
    const parameters = Array.from(new Set([...Array(26)].map(_=> 
      String.fromCharCode(Math.random()*25+65|0) )))
  
    eval(`function MyClass(${parameters.join`,`}) {` + 
      parameters.reduce((a,p) => a + `\n  this.${p} = ${p}`, '') + '\n}')
    ...
    for (let i = 0; i < 20; i++) {
      ...
      Test.assertEquals(JSON.stringify(myInstance),JSON.stringify(yourInstance),
        "JSON representation of object instances must be the same")
    }
  }
});

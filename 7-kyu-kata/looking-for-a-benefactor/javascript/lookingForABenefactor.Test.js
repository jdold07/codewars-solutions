...
describe("Fixed Tests newAvg", function () {
  it("Basic tests", function () {
    ...
    }).to.throw("Expected New Average is too low")
    expect(function () {
      newAvg([14.25, 30.76, 5.56, 7, 9, 11, 15.48, 12.987], 5)
    }).to.throw("Expected New Average is too low")
    expect(function () {
      newAvg([1400.25, 30000.76, 5.56, 7, 9, 11, 15.48, 120.98], 2000)
    }).to.throw("Expected New Average is too low")
...
describe("Random Tests 1", function () {
  for (let i = 0; i < 50; i++) {
    ...
    let sm = arr.reduce(function (a, b) {
      return a + b
    }, 0)
    let l = arr.length
    let avg = sm / l

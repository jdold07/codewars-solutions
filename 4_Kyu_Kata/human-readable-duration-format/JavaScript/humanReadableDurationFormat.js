// [object Object] - Human readable duration format  [ ID: 52742f58faf5485cae000b9a  (human-readable-duration-format) ]
...
function formatDuration (seconds) {
    ...
    function plural (units){
      switch (units[0]) {
        case "N": return //handle NaN
        case "0": return
        case "1": if (/\D/.test(units[1])) break // test for multi-digit number starting with 1, break if === 1 else continue switch to default
        default: units = units.concat("s"); break
      }
    ...
    switch (timeArray.length) {
      case 1: return timeArray.join("")
      case 2: return timeArray.join(" and ")
      default: return timeArray.join(", ").replace(/,([^,]*)$/, ` and$1`)
    }
  }

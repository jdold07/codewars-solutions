...
// Category: undefined  |  Tags: ALGORITHMS | CIPHERS | SECURITY | OBJECT-ORIENTED PROGRAMMING | STRINGS
// *****************************************************************************
function VigenèreCipher(key, abc) {
    this.encode = (str) => str
      .split("")
      .reduce((a, c, i) => a + 
        (abc.includes(c) ? abc[(abc.indexOf(c) + abc.indexOf(key[i % key.length])) % abc.length] : c), "")
    this.decode = (str) => str
      .split("")
      .reduce((a, c, i) => a + 
        (abc.includes(c) ? abc[(abc.indexOf(c) + abc.length - abc.indexOf(key[i % key.length])) % abc.length] : c), "")
  }
// *****************************************************************************
// *****************************************************************************
function VigenèreCipher(key, abc) {
    ...
    this.encode = (str) => {
      ...
      return str
        .split("")
        .reduce((a, c, i) => a + (abc.includes(c) ? abc[(abc.indexOf(c) + eMap[i]) % abc.length] : c), "")
    }
    this.decode = (str) => {

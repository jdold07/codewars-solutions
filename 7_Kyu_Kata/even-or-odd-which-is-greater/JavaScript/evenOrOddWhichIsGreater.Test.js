// [object Object] - Even or Odd - Which is Greater?  [ ID: 57f7b8271e3d9283300000b4  (even-or-odd-which-is-greater) ]
...
describe("Even or Odd - Which is Greater?", () => {
  ...
  it("Testing for 100 random tests", () => {
    ...
    for(let i = 0; i < 100; i++){
      let word = Array.from({length: randint(3, 30)}, (_, i) => randint(0, 9)).join``
      let expected = f(word)
      assert.strictEqual(evenOrOdd(word), expected, `solution(${JSON.stringify(word)}) should equal ${JSON.stringify(expected)}`);
    }
  })
});

// [object Object] - Balanced Number (Special Numbers Series #1 )   [ ID: 5a4e3782880385ba68000018  (balanced-number-special-numbers-series-number-1) ]
...
const balancedNum = (number) => {
    ...
    const numArr = number.toString().split("").map((el) => Number(el))
    ...
    return numArr.slice(0, centre).reduce((a, b) => a + b) 
      - numArr.slice(numArr.length % 2 ? centre + 1 : centre + 2).reduce((a, b) => a + b) === 0 

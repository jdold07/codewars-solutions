// [object Object] - Double Sort  [ ID: 57cc79ec484cf991c900018d  (double-sort) ]
...
describe("Tests", function(){
  it("Basic tests",function(){
    ...
    assert.deepEqual(dbSort(["Hackathon", "Katathon", "Code", "CodeWars", "Laptop", "Macbook", "JavaScript", 1, 5, 2]), [1,2,5,"Code","CodeWars","Hackathon","JavaScript","Katathon","Laptop","Macbook"]);

    assert.deepEqual(dbSort([66, 't', 101, 0, 1, 1]), [0,1,1,66,101,"t"]);
    assert.deepEqual(dbSort([78, 117, 110, 99, 104, 117, 107, 115, 4, 6, 5, 'west']), [4,5,6,78,99,104,107,110,115,117,117,"west"]);
    assert.deepEqual(dbSort([101, 45, 75, 105, 99, 107, 'y', 'no', 'yes', 1, 2, 4]), [1,2,4,45,75,99,101,105,107,"no","y","yes"]);
    ...
    assert.deepEqual(dbSort([82, 18, 72, 1, 11, 12, 12, 12, 12, 115, 667, 12, 2, 8, 3]), [1,2,3,8,11,12,12,12,12,12,18,72,82,115,667]);
    ...
    assert.deepEqual(dbSort([66, 'codewars', 11, 'alex loves pushups', 2, 3, 0]), [0,2,3,11,66,"alex loves pushups","codewars"]);
    assert.deepEqual(dbSort(['come', 'on', 110, '2500', 10, '!', 7, 15, 5, 6, 6]), [5,6,6,7,10,15,110,"!","2500","come","on"]);
    assert.deepEqual(dbSort(['when\'s', 'the', 'next', 'Katathon?', 9, 7, 0, 1, 2]), [0,1,2,7,9,"Katathon?","next","the","when's"]);
    assert.deepEqual(dbSort([8, 7, 5, 'bored', 'of', 'writing', 'tests', 115, 6, 7, 0]), [0,5,6,7,7,8,115,"bored","of","tests","writing"]);
    assert.deepEqual(dbSort(['anyone', 'want', 'to', 'hire', 'me?', 2, 4, 1]), [1,2,4,"anyone","hire","me?","to","want"]);
  ...
  it("Random tests", function(){
   ...
   for (let i = 0; i < 100; ++i){
      let size = _.random(5,30), xs = [];
      for (let j = 0; j < size; ++j){     
        let x = _.random(3,7), m = _.random(0,2);
        if (m == 1) xs.push(_.sampleSize(alph,x).join(""))
        else { let y = (_.sampleSize(digits,1)[0]).toString();

// [object Object] - String Merge!  [ ID: 597bb84522bc93b71e00007e  (string-merge) ]
...
describe( "Some random tests", () => {
  ...
  let rnd = _ => { let zz = Test.randomize(zoo); return zz.slice(-2) };
  let commonLetters = (s1,s2) => [..."ygoluritnase"].reduce( (c,l) => s1.includes(l) && s2.includes(l) ? c.concat(l) : c, [] ).filter(l=>l!=s1[0]&&l!=s2.slice(-1));
  for( let t=0; t<10; t++ ){
    let z1,z2,cc=[];
    while( cc.length==0 ){ [z1,z2] = rnd(), cc = commonLetters(z1,z2) };

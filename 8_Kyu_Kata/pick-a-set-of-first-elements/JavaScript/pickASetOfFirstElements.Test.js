// [object Object] - pick a set of first elements  [ ID: 572b77262bedd351e9000076  (pick-a-set-of-first-elements) ]
...
describe("Random tests", () =>{
  let rnd = (x=1e3) => ~~(Math.random()*x), rndOf = xx => xx[rnd(xx.length)];
  ...
  let tstArrays = _ => {
    let eee = [];
    for( let i = 0; i< 5; i++ ){ eee[i] = (eee[i-1]||[]).concat(rndArr(1)) }
    
    Test.randomize( eee ).forEach( e => {
      let arrg = e.concat(rndArr(5+rnd(5))), len = e.length;

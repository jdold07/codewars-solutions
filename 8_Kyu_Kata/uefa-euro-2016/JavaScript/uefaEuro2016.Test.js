// [object Object] - UEFA EURO 2016  [ ID: 57613fb1033d766171000d60  (uefa-euro-2016) ]
...
describe("UEFA EURO 2016", () => {
  it("Testing for fixed tests", () => {
    ...
    assert.strictEqual(uefaEuro2016(['Republic of Ireland', 'Sweden'], [1, 1]), "At match Republic of Ireland - Sweden, teams played draw.");
  })
  
  it("Random Tests:", () => {

   let listOfQualifiedTeams = ['Albania', 'Austria', 'Belgium', 'Croatia', 'Czech Republic', 'England', 'France', 'Germany', 
   ...
   for (let iteration = 0; iteration <= 50; iteration += 1) {
    ...
    let teamA = getRandomTeam(listOfQualifiedTeams), teamB = getRandomTeam(listOfQualifiedTeams), scoreA = getRandomScore(0, 10), scoreB = getRandomScore(0, 10);
    ...
    if (scoreA > scoreB) {
      assert.strictEqual(uefaEuro2016([teamA, teamB], [scoreA, scoreB]), "At match " + teamA + " - " + teamB + ", " + teamA + " won!");
    }

    if (scoreA < scoreB) {
      assert.strictEqual(uefaEuro2016([teamA, teamB], [scoreA, scoreB]), "At match " + teamA + " - " + teamB + ", " + teamB + " won!");
    }
    
    if (scoreA === scoreB) {
      assert.strictEqual(uefaEuro2016([teamA, teamB], [scoreA, scoreB]), "At match " + teamA + " - " + teamB + ", teams played draw.");
    }
   }
 })  
});

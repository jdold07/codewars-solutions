...
const { vm } = require("./versionsManager")
...
describe("VersionManager tests", () => {
  ...
  it("Major releases tests", () => {
    assert.strictEqual(vm().major().release(), "1.0.0")
    assert.strictEqual(vm("1.2.3").major().release(), "2.0.0")
    assert.strictEqual(vm("1.2.3").major().major().release(), "3.0.0")
  })

  it("Minor releases tests", () => {
    assert.strictEqual(vm().minor().release(), "0.1.0")
    assert.strictEqual(vm("1.2.3").minor().release(), "1.3.0")
    assert.strictEqual(vm("1").minor().release(), "1.1.0")
    assert.strictEqual(vm("4").minor().minor().release(), "4.2.0")
  })

  it("Patch releases tests", () => {
    assert.strictEqual(vm().patch().release(), "0.0.2")
    assert.strictEqual(vm("1.2.3").patch().release(), "1.2.4")
    assert.strictEqual(vm("4").patch().patch().release(), "4.0.2")
  })

  it("Rollbacks tests", () => {
    assert.strictEqual(vm().major().rollback().release(), "0.0.1")
    assert.strictEqual(vm().minor().rollback().release(), "0.0.1")
    assert.strictEqual(vm().patch().rollback().release(), "0.0.1")
    assert.strictEqual(vm().major().patch().rollback().release(), "1.0.0")
    assert.strictEqual(vm().major().patch().rollback().major().rollback().release(), "1.0.0")
  })
  ...
  it("Exception tests", () => {
    const checkError = (fn, error) => {
      try {
        fn()
        expect(false, "An error should be thrown")
      } catch (e) {
  ...
  describe("Random tests", () => {
    ...
    const genRandomTests = () => {
      const makeVersion = () => {
        ...
        return [...new Array(getRandomInt(0, 5))].map(() => {
      ...
      const makeSteps = () => {
        ...
        return [...new Array(Math.floor(Math.random() * 10))].map(() => list[Math.floor(Math.random() * list.length)])
      }
    ...
    it("Tests", () => {
      genRandomTests().forEach((test) => {
        assert.strictEqual(apply(vm, test), apply(referenceManager, test))
      })
    })
  })
})

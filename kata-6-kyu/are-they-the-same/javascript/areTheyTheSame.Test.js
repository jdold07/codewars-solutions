...
describe("Random tests", function () {
  ...
  for (let i in a1) if (Math.pow(a1[i],2)!=a2[i]) return false;
  return true;
}*/
  ...
  for (let i = 0; i < 200; i++) {
    ...
    let testlen = randint(10, 25)
    let a1 = [],
      a2 = []
    for (let j = 0; j < testlen; j++) {
      //elem=randint(0,100);
      let elem = randint(0, 10)
      a1.push(elem)
      a2.push(elem * elem)
    }
    // modified
    ...
    it("Testing for [" + a1.join(", ") + "] and [" + a2.join(", ") + "]", function () {
      assert.strictEqual(comp([].concat(a1), [].concat(a2)), sol(a1, a2), "It should work with random inputs too")
    })
  }
})

...
describe("GodTest", function () {
  it("Adam should be a Man", function () {
    ...
    assert.strictEqual(humans[0] instanceof Man, true, "Expected Adam to be a Man")
  })
  it("Eva should be a Woman", function () {
    let humans = God.create()
    assert.strictEqual(humans[1] instanceof Woman, true, "Expected Eva to be a Woman")
  })
  it("Adam should be a Human", function () {
    let humans = God.create()
    assert.strictEqual(humans[0] instanceof Human, true, "Expected Adam to be a Human")
  })
  it("Eva should be a Human", function () {
    let humans = God.create()
    assert.strictEqual(humans[1] instanceof Human, true, "Expected Eva to be a Human")
  })
  it("Array should only contain two items", function () {
    let humans = God.create()
    assert.strictEqual(humans.length, 2, "Expected there to only be two humans in array")
  })
})

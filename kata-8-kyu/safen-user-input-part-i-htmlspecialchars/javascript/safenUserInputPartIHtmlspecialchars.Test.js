...
const { htmlspecialchars } = require("./safenUserInputPartIHtmlspecialchars")
...
describe("Basic tests", () => {
  ...
  it("Random tests", () => {
    function externalHTMLSpecialChars(formData) {
      ...
      let i // Declare variable for "for" loop - Not required unless in strict mode
    ...
    for (let iterationVar = 0; iterationVar < 100; iterationVar++) {
      let randomString = Array.from({ length: randint(1, 1000) }, () => charsStr[randint(0, charsStr.length - 1)]).join``
      assert.strictEqual(

...
import { countPositivesSumNegatives } from "./countOfPositivesSlashSumOfNegatives"
...
describe("Testing...", function () {
  ...
  it("Random tests", function () {
    const authorSolution = function (input: number[]) {
      if (!input || input.length === 0) return []
      const xs = input as number[]
      const sum = xs.filter((e) => e < 0).length === 0 ? 0 : xs.filter((e) => e < 0).reduce((x, y) => x + y)
      return [xs.filter((e) => e > 0).length, sum]
    }
    for (let i = 0; i < 100; i++) {
      const testData: number[] = []
      const testDataSize = Math.random() * 100
      for (let y = 0; y < testDataSize; y++) {
        testData.push(Math.random() * 100)
      }
      const expected = authorSolution(testData)

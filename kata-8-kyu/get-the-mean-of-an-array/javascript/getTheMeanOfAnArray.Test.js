...
describe("Test with pre-create arrays", function () {
  ...
  it("should return the good average", function () {
    arraysTest.forEach(function (marks) {
      assert.strictEqual(getAverage(marks), so(marks), "The average of " + marks + " should return" + so(marks))
    })
  })
  it("Should work with randomly created arrays", function () {
    for (let i = 0; i < 10; i++) {
      ...
      assert.strictEqual(getAverage(testArr), so(testArr), "The average of " + testArr + " should return" + so(testArr))
    }
  })
})

...
# ******************************************************************************
from playing_with_passphrases import play_pass
...
def _():
    ...
    test.assert_equals(
        play_pass("IN 2012 TWO CAMBRIDGE UNIVERSITY RESEARCHERS ANALYSED PASSPHRASES FROM THE AMAZON PAY SYSTEM...", 20),
        "...gYnMsM SuJ HiTuGu yBn gIlZ MyMuLbJmMuJ XyMsFuHu mLyBwLuYmYl sNcMlYpChO YaXcLvGuW IqN 7897 hC",
    )
    test.assert_equals(
        play_pass("IN 2012 TWO CAMBRIDGE UNIVERSITY RESEARCHERS ANALYSED PASSPHRASES FROM THE AMAZON PAY SYSTEM...", 10),
        "...wOdCiC IkZ XyJkWk oRd wYbP CoCkBrZcCkZ NoCiVkXk cBoRmBkOcOb iDsCbOfSxE OqNsBlWkM YgD 7897 xS",
    )
    test.assert_equals(
        play_pass("1ONE2TWO3THREE4FOUR5FIVE6SIX7SEVEN8EIGHT9NINE", 5), "JsNs0yMlNj1sJaJx2cNx3jAnK4WzTk5jJwMy6tBy7jSt8"
    )
    test.assert_equals(play_pass("AZ12345678ZA", 1), "bA12345678aB")
    test.assert_equals(play_pass("!!!VPZ FWPM J", 25), "I LoVe yOu!!!")
    test.assert_equals(
        play_pass("BOY! YOU WANTED TO SEE HIM? IT'S YOUR FATHER:-)", 15), ")-:gTwIpU GjDn h'iX ?bXw tTh dI StIcPl jDn !NdQ"
    )
    test.assert_equals(
        play_pass(
    ...
    test.assert_equals(
        play_pass("ONCE UPON A TIME YOU DRESSED SO FINE (1968)", 12), ")1308( qZuR Ae pQeEqDp gAk qYuF M ZaBg qOzA"
    )
    test.assert_equals(
        play_pass("AH, YOU'VE GONE TO THE FINEST SCHOOL ALL RIGHT, MISS LONELY", 12),
        "KxQzAx eEuY ,fTsUd xXm xAaToE FeQzUr qTf aF QzAs qH'GaK ,tM",
    )
    test.assert_equals(
        play_pass("THE SPECIES, NAMED AFTER THE GREEK GOD OF THE UNDERWORLD, LIVES SOME 3,600 FEET UNDERGROUND.", 8),

...
describe("Random tests", function(){
  function sol(s) {
    if(/\d/.test(s)) {
      var t=s.split(' ').map((w,i)=>[w,i]), [n,i]=t.find(([w,i])=>+w);
  ...
  var FB = [...Array(1e5)].map((_,i)=>i+1).map(n=>n%15===0?'FizzBuzz':n%5===0?'Buzz':n%3===0?'Fizz':n);
  it("Should pass random tests", function(){
    for(let i=0; i<100; i++) {
      let l=r()*100+1|0, s=Math.max(0,r()*FB.length-l|0);

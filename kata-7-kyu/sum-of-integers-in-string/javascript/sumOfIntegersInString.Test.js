...
const { sumOfIntegersInString } = require("./sumOfIntegersInString")
...
it("Example test cases", () => {
  ...
  exampleTests.forEach(([testString, correctAnswer]) => assert.strictEqual(sumOfIntegersInString(testString), correctAnswer))
})
...
describe("Random tests", () => {
  let rnd = (x) => ~~(Math.random() * x),
    ...
    rndStr = () =>
      [...Array(3 + rnd(5))].map(() => rndOf("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ;,?:!§%()#[]")).join``
  ...
  for (let x = 0; x < 99; x++) {
    let data = rndStr(),
      ...
      r,
      rrr = []
    for (let z = 0; z < 1 + rnd(9); z++) {
      r = rnd(1e4)
      rrr.push(r)
      data += r + rndStr()
      exp += r
    }
    it(`Should sum numbers in "${data}" : (=${rrr.join("+")})`, () => {
      assert.strictEqual(sumOfIntegersInString(data), exp)
    })
  }
})

...
from double_sort import db_sort
...
def fixed_tests():
    ...
    def basic_test_cases():
        ...
        test.assert_equals(
            db_sort(["Hackathon", "Katathon", "Code", "CodeWars", "Laptop", "Macbook", "JavaScript", 1, 5, 2]),
            [1, 2, 5, "Code", "CodeWars", "Hackathon", "JavaScript", "Katathon", "Laptop", "Macbook"],
        )
        test.assert_equals(db_sort([66, "t", 101, 0, 1, 1]), [0, 1, 1, 66, 101, "t"])
        test.assert_equals(
            db_sort([78, 117, 110, 99, 104, 117, 107, 115, 4, 6, 5, "west"]),
            [4, 5, 6, 78, 99, 104, 107, 110, 115, 117, 117, "west"],
        )
        test.assert_equals(
            db_sort([101, 45, 75, 105, 99, 107, "y", "no", "yes", 1, 2, 4]),
            [1, 2, 4, 45, 75, 99, 101, 105, 107, "no", "y", "yes"],
        )
        test.assert_equals(
            db_sort([80, 117, 115, 104, 45, 85, 112, 115, 6, 7, 2]), [2, 6, 7, 45, 80, 85, 104, 112, 115, 115, 117]
        )
        test.assert_equals(
            db_sort([1, 1, 1, 1, 1, 2, "1", "2", "three", 1, 2, 3]), [1, 1, 1, 1, 1, 1, 2, 2, 3, "1", "2", "three"]
        )
        ...
        test.assert_equals(
            db_sort([82, 18, 72, 1, 11, 12, 12, 12, 12, 115, 667, 12, 2, 8, 3]),
            [1, 2, 3, 8, 11, 12, 12, 12, 12, 12, 18, 72, 82, 115, 667],
        )
        ...
        test.assert_equals(
            db_sort([66, "codewars", 11, "alex loves pushups", 2, 3, 0]), [0, 2, 3, 11, 66, "alex loves pushups", "codewars"]
        )
        test.assert_equals(
            db_sort(["come", "on", 110, "2500", 10, "!", 7, 15, 5, 6, 6]),
            [5, 6, 6, 7, 10, 15, 110, "!", "2500", "come", "on"],
        )
        test.assert_equals(
            db_sort(["when's", "the", "next", "Katathon?", 9, 7, 0, 1, 2]),
            [0, 1, 2, 7, 9, "Katathon?", "next", "the", "when's"],
        )
        test.assert_equals(
            db_sort([8, 7, 5, "bored", "of", "writing", "tests", 115, 6, 7, 0]),
            [0, 5, 6, 7, 7, 8, 115, "bored", "of", "tests", "writing"],
        )
        test.assert_equals(
            db_sort(["anyone", "want", "to", "hire", "me?", 2, 4, 1]), [1, 2, 4, "anyone", "hire", "me?", "to", "want"]
        )
        test.assert_equals(
    ...
    def _():
        for _ in range(0, 100):
            ...
            for j in range(10):
                x = ran(3, 8)
                if ran(0, 3) == 1:
                    xs.append(("".join(sample(abc, x)).capitalize()))
                else:
                    y = sample(digits, 1)[0]
                    if ran(0, 2) == 0:
                        xs.append(int(y))

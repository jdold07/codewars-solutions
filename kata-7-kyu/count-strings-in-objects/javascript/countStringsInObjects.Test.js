...
const { strCount } = require("./countStringsInObjects")
...
function fm88(obj) {
  ...
  function counter(obj) {
    for (let key in obj) {
      ...
      else if (value instanceof Array) counter(value)
      else if (value instanceof Object) counter(value)
    }
    return count
  }
  return counter(obj)
...
describe("Fixed Tests", () => {
  it("test", () => {
    ...
    assert.strictEqual(
      strCount(["1", 2, "3", "4"]),
      3,
      "Did not count the correct number of strings. Check counting inside nested objects. Did you start counting from 0?"
    )
    assert.strictEqual(
      strCount(["1", 2, "3", ["and another", 2, "and again", [undefined, "and again"], false], "4"]),
      6,
      "Did not count the correct number of items."
    )
    assert.strictEqual(
      strCount({
        one: "1",
      ...
      5,
      "Did not count the correct number of strings."
    )
  })
})
...
describe("Random Tests", () => {
  it("test", () => {
    let arr1 = [],
      a = Math.floor(Math.random(0, 1) * 50)
    for (let i = 0; i < a; i++) {
      arr1.push("strin")
    }
    ...
    let arr2 = [],
      b = Math.floor(Math.random(0, 1) * 50)
    for (let i = 0; i < b; i++) {
      arr2.push("strin")
    }

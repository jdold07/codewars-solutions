...
const { cypher } = require("./numberEncryptingCypher")
...
describe('The function "cypher"', function () {
  it("should pass the basic tests", function () {
    assert.strictEqual(cypher("Hello World"), "H3110 W0r1d", 'Testing "Hello World"')
    assert.strictEqual(cypher("I am your father"), "1 4m y0ur f47h3r", 'Testing "I am your father"')
    assert.strictEqual(
    ...
    assert.strictEqual(cypher("IlRzEeAaSsGbTtBgOo"), "112233445566778900", 'Testing "IlRzEeAaSsGbTtBgOo"')
  })
  ...
  it("should pass 20 random test (10-20 words)", function () {
    for (let i = 0; i < 20; ++i) {
      ...
      assert.strictEqual(cypher(string), Solution.getResult(string), 'Testing "' + string + '"')
    }
  })

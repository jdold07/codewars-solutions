...
const { getStrings } = require("./interviewQuestionEasy")
...
describe("Fixed tests", function () {
  it("Tests", function () {
    ...
    assert.equal(
      getStrings("Llanfairpwllgwyngyllgogerychwyrndrobwllllantysiliogogogoch"),
...
function solution(city) {
  const o = {},
    res = []
  for (const c of city.toLowerCase().replace(/[^a-z]/gi, "")) o[c] = o[c] ? o[c] + "*" : c + ":*"
  for (const k in o) res.push(o[k])
  return res.join(",")
}
...
describe("Random Tests", function () {
  it("Tests", function () {
    ...
    shuffle(cities)
    for (let x of cities) assert.strictEqual(getStrings(x), solution(x))
  })
})

...
const { encode } = require("./digitalCypher")

describe("Encoding..", function () {
  it("Basic Tests", function () {
    assert.strictEqual(encode("scout", 1939).join(), [20, 12, 18, 30, 21].join())
    assert.strictEqual(encode("masterpiece", 1939).join(), [14, 10, 22, 29, 6, 27, 19, 18, 6, 12, 8].join())
  })
  ...
  it("Random Tests", function () {
    ...
    while (loop < 50000) {
      let str = makeStr()
      assert.strictEqual(encode(str, loop + 2).join(), e(str, loop + 2).join())
      loop += 123
    }
  })
})

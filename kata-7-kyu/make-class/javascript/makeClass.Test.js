...
describe("Basic Tests", function () {
  ...
  it("anti-cheat", function () {
    assert.strictEqual(dog2 instanceof Animel, true, '"instances" of your class need to be instances of your class.')
...
it("Some random Tests", function () {
  for (let i = 0; i < 50; i++) {
    ...
    assert.strictEqual(JSON.stringify(o), JSON.stringify(p), "JSON representation of object instances must be the same")
...
it("Some more random Tests", function () {
  for (let i = 0; i < 20; i++) {
    const parameters = Array.from(new Set([...Array(26)].map(() => String.fromCharCode((Math.random() * 25 + 65) | 0))))

...
describe("Random tests", () => {
  const sol = (classPoints: number[], yourPoints: number) => classPoints.reduce((a, b) => a + b, 0) / classPoints.length < yourPoints;
  for (let i = 0; i < 100; i++) {
    const arr = Array.from({ length: 50 }, () => Math.floor(Math.random() * 100));
    const points = Math.floor(Math.random() * 100);
    const exp = sol(arr, points);
    it(`betterThanAverage([${arr.join(', ')}], ${points})`, () => assert[exp ? "isTrue" : "isFalse"](betterThanAverage(arr, points)));
  }
});

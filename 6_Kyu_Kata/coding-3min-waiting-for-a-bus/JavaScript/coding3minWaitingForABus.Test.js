// [object Object] - Coding 3min : Waiting for a Bus  [ ID: 57070eff924f343280000015  (coding-3min-waiting-for-a-bus) ]
...
function an(time,km){
  let start=360+km*2, end=1200+km*2, [hh,mm]=time.split(":").map(x=>+x), wait=hh*60+mm;
  if (end<wait) wait=start    
  while(start<wait) start+=15
  start%=1440
  return ("0"+~~(start/60)).substr(-2)+":"+("0"+start%60).substr(-2)
}
function sc2(time,km){
  let [hh,mm]=time.split(":").map(x=>+x), travel=km*2, total;
  if (hh<6) hh=6,mm=0
  let wait=hh*60+mm;
...
Test.assertSimilar(sc('06:00',1  ),'06:02', "good luck!");  
Test.assertSimilar(sc('06:00',10 ),'06:20', "good luck!");  
Test.assertSimilar(sc('07:00',10 ),'07:05', "good luck!");  
Test.assertSimilar(sc('10:00',100),'10:05', "good luck!"); 
Test.assertSimilar(sc('09:00',100),'09:20', "good luck!");  
Test.assertSimilar(sc('05:00',1  ),'06:02', "good luck!");
Test.assertSimilar(sc('20:00',1  ),'20:02', "good luck!"); 
Test.assertSimilar(sc('20:03',1  ),'06:02', "good luck!");  
Test.assertSimilar(sc('23:58',420),'00:00', "good luck!");
Test.assertSimilar(sc('10:54',72 ),'10:54', "good luck!");
Test.assertSimilar(sc('22:23',244),'22:23', "good luck!");
...
for (let myjinxini=0;myjinxini<100;myjinxini++){
  let tt=rndtime(), kk=rndkm(), answer=an(tt,kk);
  console.log("<font color='#CD7F32'>Test: time='"+tt+"'  km="+kk+"</font>","")
  Test.assertSimilar(sc(tt,kk), answer, "good luck!");
}
